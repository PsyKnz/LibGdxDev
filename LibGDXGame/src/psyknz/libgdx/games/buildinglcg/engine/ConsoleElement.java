...
public class ConsoleElement implements GameElement {
	...
	public final Array<TextElement> log;
	...
	public void draw(SpriteBatch batch, int xOffset, int yOffset) {
		if(visible) {
			...
			font.draw(batch, fpsLabel + fps, game.getScreen().viewport.x, game.getScreen().viewport.height - game.getScreen().viewport.y);

			// Draws the GameScreens ID to the screen.
			font.draw(batch, game.getScreen().toString(), game.getScreen().viewport.x, 16 - game.getScreen().viewport.y);

			// Draws all logged messages to the screen, starting with the latest at the bottom of the screen.
		    for(int i = 0; i < log.size; i++) {
			    //log.get(i).draw(batch, game.getScreen().viewport.x + game.getScreen().viewport.y, 0 - game.getScreen().viewport.y + i * 18 + 4);
		    }
		}
	}
