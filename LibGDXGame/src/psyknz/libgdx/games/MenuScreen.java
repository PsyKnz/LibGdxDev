...
public class MenuScreen extends GameScreen implements ElementListener {
	...
	// Disposes of all assets generated by the MenuScreen.
	@Override
	public void dispose() {
		super.dispose();
		tex.dispose();
	...
	// Recieves input from GameElements on the screen.
	@Override
	public void event(ElementEvent event) {
		// If the "Play" button is touched then the PlayScreen is loaded.
		if(event.element == playButton) {
			if(event.type == ElementEvent.EventType.TOUCHED) setScreen(new PlayScreen(game));
		}
	}
}
